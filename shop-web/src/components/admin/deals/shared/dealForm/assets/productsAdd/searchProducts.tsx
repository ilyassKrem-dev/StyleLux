...
export default function SearchProducts({handleClick}:{
    handleClick:(arg:ProductType) => void
}) {
    ...
    return (
        <div className="mt-4 relative p-3 addbar">
            <div className="relative flex justify-center items-center" onClick={() => setShow(true)}>
                <input type="text" id="search" name="search" className=" border !p-1 rounded-full !text-base !pl-8 w-full" placeholder="Add product" value={input} onChange={(e) => setInput(e.target.value)} />
                <div className="absolute text-2xl left-2">
                    <IoIosSearch />
                </div>
            </div>
            {show&&products.length>0&&
            <div className="absolute top-[3.5rem] w-full rounded-md  shadow-xl z-20 pr-5">
                <div className="w-full bg-white dark:bg-dark text-black/80 dark:text-white overflow-y-auto custom-scrollbar max-h-[350px] flex flex-col  border-2 border-black/10 dark:border-white/10 rounded-md " ref={divRef} onScroll={handleScroll}>
                    {products.map((product,index) => {
                        const {media,name,price} = product
                        return (
                        <div key={index} className={`flex items-center gap-3 cursor-pointer group hover:bg-black/10 dark:hover:bg-white/10 ${index==0 ?" rounded-tl-md" :"border-t dark:border-t-white/20"} `} onClick={() => {
                            handleClick(product)
                            setShow(false)}}>
