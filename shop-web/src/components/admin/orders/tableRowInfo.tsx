...
export default function TableRowInfo({order}:{
    ...
    const {id,uid,userName,userId,total,items} = order
    const {w} = useSize()
    return (
        <tr >
            ...
            <td className="flex-1 border-r p-2 w-[80px] dark:border-white/50 border-black/50 max-[400px]:hidden text-center dark:text-white">
                ${total.toFixed(1)}
            </td>
            <td className="flex-1 flex justify-center items-center p-2 w-[120px]">
                <Link to={`/admin/orders/${uid}`}>
